...
The NHTS reports mode chosen per trip reported. Below is a table that shows the distribution of the mode choice for each population of ability.
...
persons_trips %>% group_by(Ability, trptrans) %>%
  ...
  spread(Ability, `Distribution(%)`) %>% pander()
```

As some of the modes are irrelevant to the study, the mode choices will be aggregated into categories. The NHTS asks respondents if 
```{r aggregate.mode, out.width = "120%", fig.align = "center"}
