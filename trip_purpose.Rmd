...
# find all trips that belong to an hhpersonid groups them by purpose and counts them
...
```{r trip.purpose, message=F, warning=F, out.width = "120%", fig.align = "center"}
...
This plot shows the same distribution of number of trips by purpose, but includes the income groups. 
...
Note the changes in number of trips for wheelchair users as income increases. For home-based work trips, the percent of users who make zero trips decreases, while the number of zero trip
...
# plot the changes in number of trips over time for each purpose, income group, and ability.
ggplot(joined_grid_trips, aes(x = hhfaminc, y = stat(width*density))) +
